...
class CustomTrainer(Trainer):
    ...
    def __init__(
        ...
        fit_loop = FitLoop(
            min_epochs=(
                1
                if (min_epochs is None and min_steps is None and max_time is None)
                else min_epochs
            ),
            max_epochs=(
                max_epochs
                if max_epochs is not None
                else (1000 if (max_steps == -1 and max_time is None) else -1)
            ),
        )
        log.info(f"Current line in trainer: {sys._getframe().f_lineno}")
        ...
        if terminate_on_nan is not None:
            ...
            if not isinstance(terminate_on_nan, bool):
                raise TypeError(
                    f"`terminate_on_nan` should be a bool, got {terminate_on_nan}."
                )
        log.info(f"Current line in trainer: {sys._getframe().f_lineno}")
        ...
        if track_grad_norm != -1 and not (
            (isinstance(track_grad_norm, (int, float)) or track_grad_norm == "inf")
            and float(track_grad_norm) > 0
        ):
            raise MisconfigurationException(
        ...
        self.logger_connector.on_trainer_init(
            logger, flush_logs_every_n_steps, log_every_n_steps, move_metrics_to_cpu
        )
        log.info(f"Current line in trainer: {sys._getframe().f_lineno}")
    ...
    def _init_debugging_flags(
        ...
        self.limit_predict_batches = _determine_batch_limits(
            limit_predict_batches, "limit_predict_batches"
        )
        self.val_check_interval = _determine_batch_limits(
            val_check_interval, "val_check_interval"
        )
        self.overfit_batches = _determine_batch_limits(
    ...
    def _call_and_handle_interrupt(
        self, trainer_fn: Callable, *args: Any, **kwargs: Any
    ) -> Any:
        r"""
    ...
    def validate(
        ...
        return self._call_and_handle_interrupt(
            self._validate_impl, model, dataloaders, ckpt_path, verbose, datamodule
        )
    ...
    def test(
        ...
        return self._call_and_handle_interrupt(
            self._test_impl, model, dataloaders, ckpt_path, verbose, datamodule
        )
    ...
    def _restore_modules_and_callbacks(
        self, checkpoint_path: Optional[_PATH] = None
    ) -> None:
    ...
    def _log_hyperparams(self) -> None:
        ...
        if self.logger is not None:
            ...
            datamodule_log_hyperparams = (
                self.datamodule._log_hyperparams
    ...
    def _run_evaluate(self) -> _EVALUATE_OUTPUT:
        ...
        with self.profiler.profile(
            f"run_{self.state.stage}_evaluation"
        ), torch.no_grad():
    ...
    def _run_sanity_check(self, ref_model):
        using_val_step = (
            ...
            and is_overridden("validation_step", ref_model)
        )
        should_sanity_check = (
            using_val_step
    ...
    def __set_ckpt_path(
        self, ckpt_path: Optional[str], model_provided: bool, model_connected: bool
    ) -> Optional[str]:
    ...
    def _handle_meta_model(self) -> None:
        ...
        if isinstance(self.training_type_plugin, DDPSpawnPlugin):
            raise MisconfigurationException(
                "LightningModule on meta device isn't supported with spawn."
            )
    ...
    def call_hook(
        ...
        **kwargs: Any,
        ...
        with self.profiler.profile(hook_name):
            ...
            if hook_name in ("on_train_start",) and hasattr(
                self.accelerator, hook_name
            ):
    ...
    def __setup_profiler(self) -> None:
        ...
        self.profiler.setup(
            stage=self.state.fn._setup_fn, local_rank=local_rank, log_dir=self.log_dir
        )
    ...
    def _log_device_info(self) -> None:
        ...
        num_tpu_cores = (
            self.tpu_cores
            if self.tpu_cores is not None and self._device_type == DeviceType.TPU
            else 0
        )
        rank_zero_info(
            f"TPU available: {_TPU_AVAILABLE}, using: {num_tpu_cores} TPU cores"
        )
    ...
    def enable_validation(self) -> bool:
        ...
        val_loop_enabled = (
            is_overridden("validation_step", model_ref) and self.limit_val_batches > 0
        )
    ...
    def resume_from_checkpoint(self) -> Optional[Union[str, Path]]:
        resume_from_checkpoint = (
            self.checkpoint_connector.resume_from_checkpoint_fit_path
        )
    ...
    def from_argparse_args(
        cls: Any, args: Union[Namespace, ArgumentParser], **kwargs
    ) -> Any:
    ...
    def _exit_gracefully_on_signal(self) -> None:
        if _fault_tolerant_training() and self._terminate_gracefully:
            ...
            raise ExitGracefullyException(
                f"Exiting gracefully on {class_name}:{caller.function}"
            )

    @property
    def weights_summary(self) -> Optional[str]:
        rank_zero_deprecation(
            "`Trainer.weights_summary` is deprecated in v1.5 and will be removed in v1.7."
        )
    ...
    def weights_summary(self, val: Optional[str]) -> None:
        rank_zero_deprecation(
            "Setting `Trainer.weights_summary` is deprecated in v1.5 and will be removed in v1.7."
        )
    ...
    def terminate_on_nan(self) -> bool:
        rank_zero_deprecation(
            "`Trainer.terminate_on_nan` is deprecated in v1.5 and will be removed in 1.7."
        )
